...
// Возвращает (оставшуюся часть входной строки, распарсенное содержимое без кавычек)
...
fn do_unquote_non_escaped<'a>(input: &'a str) -> Result<(&'a str, &'a str), ()> {
    ...
    if 0 == quote_byteidx || input.as_bytes().get(quote_byteidx - 1) == Some(&b'\\') {
        return Err(());
    }
    Ok((
        &input[1 + quote_byteidx..], // остаток после закрывающей кавычки
        &input[..quote_byteidx],     // содержимое кавычек
    ))
}
...
impl<T: Parser> Parser for List<T> {
    ...
    fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ()> {
        ...
        loop {
            input = input.trim_start();
            // Проверяем, не закрывающая ли скобка
            if let Some(remaining) = input.strip_prefix(']') {
                return Ok((remaining.trim_start(), result));
            }
            // Парсим один элемент
            let (new_input, item) = self.parser.parse(input)?;
            result.push(item);
            input = new_input.trim_start();
            // После элемента ожидаем либо запятую, либо ']'
            if let Some(after_comma) = input.strip_prefix(',') {
                input = after_comma;
            } else if !input.starts_with(']') {
                return Err(()); // нет ни запятой, ни конца списка
            }
        }
    }
}
...
impl Parsable for Backet {
    ...
    fn parser() -> Self::Parser {
        map(
            ...
            |(asset_id, count): (String, NonZeroU32)| Backet {
                asset_id,
                count: count.get(),
            },
        )
...
impl Parsable for UserCash {
    ...
    fn parser() -> Self::Parser {
        map(
            ...
            |(user_id, count): (String, NonZeroU32)| UserCash {
                user_id,
                count: count.get(),
            },
        )
...
pub fn parse_parsable<T: Parsable>(input: &str) -> Result<(&str, T), ()> {
...
/// Trace [приложения](AppLogKind) — обёрнули крупный вариант в Box
#[derive(Debug, Clone, PartialEq)]
...
pub enum AppLogJournalKind {
    ...
    DepositCash(Box<UserCash>),
    WithdrawCash(Box<UserCash>),
    BuyAsset(Box<UserBacket>),
    SellAsset(Box<UserBacket>),
}
        ...
        preceded(
            ...
            alt8(
                map(
                    ...
                    |(user_id, authorized_capital): (String, NonZeroU32)| {
                        AppLogJournalKind::CreateUser {
                            user_id,
                            authorized_capital: authorized_capital.get(),
                        }
                    },
                ),
                map(
                ...
                map(
                    ...
                    |(asset_id, user_id, liquidity): (String, String, NonZeroU32)| {
                ...
                map(
                    ...
                    |user_cash| AppLogJournalKind::DepositCash(Box::new(user_cash)),
                ),
                map(
                    preceded(strip_whitespace(tag("WithdrawCash")), UserCash::parser()),
                    |user_cash| AppLogJournalKind::WithdrawCash(Box::new(user_cash)),
                ),
                map(
                    preceded(strip_whitespace(tag("BuyAsset")), UserBacket::parser()),
                    |user_backet| AppLogJournalKind::BuyAsset(Box::new(user_backet)),
                ),
                map(
                    preceded(strip_whitespace(tag("SellAsset")), UserBacket::parser()),
                    |user_backet| AppLogJournalKind::SellAsset(Box::new(user_backet)),
                ),
            ),
        )
    }
...
impl Parsable for LogLine {
    ...
    fn parser() -> Self::Parser {
        map(
            ...
            |(kind, request_id): (LogKind, NonZeroU32)| LogLine {
                kind,
...
mod test {
    ...
    fn test_i32() {
        ...
        assert_eq!(stdp::I32.parse("0x03"), Ok(("x03", 0)));
        assert_eq!(stdp::I32.parse("-"), Err(()));
    }
    ...
    fn test_delimited() {
        assert_eq!(
            ...
            Ok(("", NonZeroU32::new(50).unwrap()))
        );
        assert_eq!(
            delimited(tag("["), stdp::U32, tag("]")).parse("[0x32] nice"),
            Ok((" nice", NonZeroU32::new(50).unwrap()))
        );
        assert_eq!(
            delimited(tag("["), stdp::U32, tag("]")).parse("0x32]"),
            Err(())
